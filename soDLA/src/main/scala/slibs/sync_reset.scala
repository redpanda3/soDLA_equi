...
@chiselName
class SO_sync_reset extends Module {
    ...
    val UJ_dft_xclamp_ctrl_cdc_sync_reset = Module(new SO_BLKBOX_SRC0)
    dft_xclamp_ctrl_cdc_sync_reset := UJ_dft_xclamp_ctrl_cdc_sync_reset.io.Y


    val UJ_inreset_x_clamp = Module(new SO_OR2D1)
    ...
    val UI_test_mode_inmux = Module(new SO_MUX2D4)
    ...
    val UI_test_mode_outmux = Module(new SO_MUX2D4)
