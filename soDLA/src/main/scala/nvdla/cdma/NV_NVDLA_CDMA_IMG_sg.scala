...
@chiselName
class NV_soDLA_CDMA_IMG_sg(implicit conf: nvdlaConfig) extends Module {
...
////////////////////////////////////////////////////////////////////////
    ...
    val NV_soDLA_PDP_RDMA_rdreq = Module{new NV_soDLA_DMAIF_rdreq(conf.NVDLA_CDMA_MEM_RD_REQ)}
    NV_soDLA_PDP_RDMA_rdreq.io.nvdla_core_clk := io.nvdla_core_clk
    NV_soDLA_PDP_RDMA_rdreq.io.reg2dp_src_ram_type := io.reg2dp_datain_ram_type
    if(conf.NVDLA_SECONDARY_MEMIF_ENABLE){
        io.img_dat2cvif_rd_req_pd.get <> NV_soDLA_PDP_RDMA_rdreq.io.cvif_rd_req_pd.get
    }
    io.img_dat2mcif_rd_req_pd <> NV_soDLA_PDP_RDMA_rdreq.io.mcif_rd_req_pd

    NV_soDLA_PDP_RDMA_rdreq.io.dmaif_rd_req_pd.bits := dma_rd_req_pd
    NV_soDLA_PDP_RDMA_rdreq.io.dmaif_rd_req_pd.valid := dma_rd_req_vld
    dma_rd_req_rdy := NV_soDLA_PDP_RDMA_rdreq.io.dmaif_rd_req_pd.ready
    // rd Channel: Response
    val NV_soDLA_PDP_RDMA_rdrsp = Module{new NV_soDLA_DMAIF_rdrsp(conf.NVDLA_CDMA_MEM_RD_RSP)}
    NV_soDLA_PDP_RDMA_rdrsp.io.nvdla_core_clk := io.nvdla_core_clk
    if(conf.NVDLA_SECONDARY_MEMIF_ENABLE){
        NV_soDLA_PDP_RDMA_rdrsp.io.cvif_rd_rsp_pd.get <> io.cvif2img_dat_rd_rsp_pd.get
    }
    NV_soDLA_PDP_RDMA_rdrsp.io.mcif_rd_rsp_pd <> io.mcif2img_dat_rd_rsp_pd

    dma_rd_rsp_pd := NV_soDLA_PDP_RDMA_rdrsp.io.dmaif_rd_rsp_pd.bits
    dma_rd_rsp_vld := NV_soDLA_PDP_RDMA_rdrsp.io.dmaif_rd_rsp_pd.valid
    NV_soDLA_PDP_RDMA_rdrsp.io.dmaif_rd_rsp_pd.ready := dma_rd_rsp_rdy
    ...
    val u_NV_soDLA_CDMA_IMG_fifo = Module{new NV_soDLA_fifo(depth = 128, width = 11,
                                    ram_type = 2, 
                                    distant_wr_req = true)}
    u_NV_soDLA_CDMA_IMG_fifo.io.clk := io.nvdla_core_clk
    u_NV_soDLA_CDMA_IMG_fifo.io.pwrbus_ram_pd := io.pwrbus_ram_pd
    u_NV_soDLA_CDMA_IMG_fifo.io.wr_pvld := dma_req_fifo_req
    dma_req_fifo_ready := u_NV_soDLA_CDMA_IMG_fifo.io.wr_prdy
    u_NV_soDLA_CDMA_IMG_fifo.io.wr_pd := dma_req_fifo_data
    val dma_rsp_fifo_req = u_NV_soDLA_CDMA_IMG_fifo.io.rd_pvld
    u_NV_soDLA_CDMA_IMG_fifo.io.rd_prdy := dma_rsp_fifo_ready
    val dma_rsp_fifo_data = u_NV_soDLA_CDMA_IMG_fifo.io.rd_pd
    ...
    val u_NV_soDLA_CDMA_IMG_sg2pack_fifo = Module{new NV_soDLA_fifo(depth = 128, width = 11,
                                            ram_type = 1, 
                                            distant_wr_req = false)}
    u_NV_soDLA_CDMA_IMG_sg2pack_fifo.io.clk := io.nvdla_core_clk
    u_NV_soDLA_CDMA_IMG_sg2pack_fifo.io.pwrbus_ram_pd := io.pwrbus_ram_pd

    u_NV_soDLA_CDMA_IMG_sg2pack_fifo.io.wr_pvld := sg2pack_push_req
    val sg2pack_push_ready = u_NV_soDLA_CDMA_IMG_sg2pack_fifo.io.wr_prdy
    u_NV_soDLA_CDMA_IMG_sg2pack_fifo.io.wr_pd := sg2pack_push_data
    sg2pack_pop_req := u_NV_soDLA_CDMA_IMG_sg2pack_fifo.io.rd_pvld
    u_NV_soDLA_CDMA_IMG_sg2pack_fifo.io.rd_prdy := sg2pack_pop_ready
    sg2pack_pop_data := u_NV_soDLA_CDMA_IMG_sg2pack_fifo.io.rd_pd
...
object NV_soDLA_CDMA_IMG_sgDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_CDMA_IMG_sg())
}
