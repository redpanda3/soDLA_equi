...
@chiselName
class NV_soDLA_CDMA_dcIO(implicit conf: nvdlaConfig) extends Bundle{
...
class NV_soDLA_CDMA_dc(implicit conf: nvdlaConfig) extends Module {
    val io = IO(new NV_soDLA_CDMA_dcIO)
...
///////////// stage 1 /////////////}
    ...
    io.dc_dat2mcif_rd_req_pd <> NV_soDLA_PDP_RDMA_rdreq.io.mcif_rd_req_pd
    if(conf.NVDLA_SECONDARY_MEMIF_ENABLE){
        io.dc_dat2cvif_rd_req_pd.get <> NV_soDLA_PDP_RDMA_rdreq.io.cvif_rd_req_pd.get 
    }
    ...
    NV_soDLA_PDP_RDMA_rdrsp.io.mcif_rd_rsp_pd <> io.mcif2dc_dat_rd_rsp_pd
    if(conf.NVDLA_SECONDARY_MEMIF_ENABLE){
        NV_soDLA_PDP_RDMA_rdrsp.io.cvif_rd_rsp_pd.get <> io.cvif2dc_dat_rd_rsp_pd.get
    }
    
    val dma_rd_rsp_pd = NV_soDLA_PDP_RDMA_rdrsp.io.dmaif_rd_rsp_pd.bits
    val dma_rd_rsp_vld = NV_soDLA_PDP_RDMA_rdrsp.io.dmaif_rd_rsp_pd.valid
...
object NV_soDLA_CDMA_dcDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_CDMA_dc())
}
