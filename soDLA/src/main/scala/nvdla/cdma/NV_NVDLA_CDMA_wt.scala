...
@chiselName
class NV_soDLA_CDMA_wtIO(implicit conf: nvdlaConfig) extends Bundle {
...
class NV_soDLA_CDMA_wt(implicit conf: nvdlaConfig) extends Module {
    val io = IO(new NV_soDLA_CDMA_wtIO)
//     
//          ┌─┐       ┌─┐
//       ┌──┘ ┴───────┘ ┴──┐
//       │                 │
...
// rd Channel: Request
    ...
    NV_soDLA_PDP_RDMA_rdreq.io.reg2dp_src_ram_type := io.reg2dp_weight_ram_type
    if(conf.NVDLA_SECONDARY_MEMIF_ENABLE){
        io.cdma_wt2cvif_rd_req_pd.get <> NV_soDLA_PDP_RDMA_rdreq.io.cvif_rd_req_pd.get
    }
    io.cdma_wt2mcif_rd_req_pd <> NV_soDLA_PDP_RDMA_rdreq.io.mcif_rd_req_pd
    ...
    if(conf.NVDLA_SECONDARY_MEMIF_ENABLE){
        NV_soDLA_PDP_RDMA_rdrsp.io.cvif_rd_rsp_pd.get <> io.cvif2cdma_wt_rd_rsp_pd.get
    }
    NV_soDLA_PDP_RDMA_rdrsp.io.mcif_rd_rsp_pd <> io.mcif2cdma_wt_rd_rsp_pd
...
//continue reading 8*atmm data from memory and then Cbuf can be fully written 
    ...
    val u_8atmm_fifo = Module{new NV_soDLA_fifo(depth = 8*conf.ATMM/conf.DMAIF, width = conf.NVDLA_CDMA_MEM_RD_RSP,
                        ram_type = 2, 
                        distant_wr_req = false)}
    u_8atmm_fifo.io.clk := io.nvdla_core_clk
    u_8atmm_fifo.io.pwrbus_ram_pd := io.pwrbus_ram_pd
    u_8atmm_fifo.io.wr_pvld := NV_soDLA_PDP_RDMA_rdrsp.io.dmaif_rd_rsp_pd.bits
    NV_soDLA_PDP_RDMA_rdrsp.io.dmaif_rd_rsp_pd.ready := u_8atmm_fifo.io.wr_prdy
    u_8atmm_fifo.io.wr_pd := NV_soDLA_PDP_RDMA_rdrsp.io.dmaif_rd_rsp_pd.bits
...
///////////////////////////////////
    ...
    val u_fifo = Module{new  NV_soDLA_fifo(depth = 128, width = 6,
                        ram_type = 2, 
                        distant_wr_req = true)}
...
object NV_soDLA_CDMA_wtDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_CDMA_wt())
}
