...
@chiselName
class NV_soDLA_CDMA_cvt(implicit conf: nvdlaConfig) extends Module {
...
val u_cell = Array.fill(conf.BNUM){Module(new NV_soDLA_CDMA_CVT_cell)}
for(i <- 0 to conf.BNUM-1){
...
object NV_soDLA_CDMA_cvtDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_CDMA_cvt())
}
