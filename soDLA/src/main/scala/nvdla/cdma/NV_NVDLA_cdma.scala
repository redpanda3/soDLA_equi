...
@chiselName
class NV_soDLA_cdmaIO(implicit val conf: nvdlaConfig) extends Bundle{
...
class NV_soDLA_cdma(implicit val conf: nvdlaConfig) extends Module {
    val io = IO(new NV_soDLA_cdmaIO)
//     
//          ┌─┐       ┌─┐
//       ┌──┘ ┴───────┘ ┴──┐
//       │                 │
...
withReset(io.nvdla_core_rstn){
    ...
    val u_regfile = Module(new NV_soDLA_CDMA_regfile)
    val u_wt = Module(new NV_soDLA_CDMA_wt)   
    val u_slcg_wt = Module(new NV_soDLA_slcg(1, false))
    val u_dc = Module(new NV_soDLA_CDMA_dc)
    val u_slcg_dc = Module(new NV_soDLA_slcg(3, false))
    val u_img = Module(new NV_soDLA_CDMA_img)
    val u_slcg_img = Module(new NV_soDLA_slcg(3, false))
    val u_dma_mux = Module(new NV_soDLA_CDMA_dma_mux)
    val u_slcg_mux = Module(new NV_soDLA_slcg(1, false))
    val u_cvt = Module(new NV_soDLA_CDMA_cvt)
    val u_slcg_cvt = Module(new NV_soDLA_slcg(1, false))
    val u_slcg_hls = Module(new NV_soDLA_slcg(2, false))
    val u_shared_buffer = Module(new NV_soDLA_CDMA_shared_buffer)
    val u_slcg_buffer = Module(new NV_soDLA_slcg(1, false))
    val u_status = Module(new NV_soDLA_CDMA_status)
    //==========================================================
    // Regfile
    //==========================================================
...
object NV_soDLA_cdmaDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_cdma())
}
