...
@chiselName
class NV_soDLA_sdp(implicit val conf: nvdlaConfig) extends Module {
...
withReset(!io.nvdla_core_rstn){  
    ...
    //DMA
    //--------------------------------------- 
    val u_rdma = Module(new NV_soDLA_SDP_rdma)
    val u_wdma = Module(new NV_soDLA_SDP_wdma)
    //========================================
    //SDP core instance
    //----------------------------------------
    val u_core = Module(new NV_soDLA_SDP_core)
    //=======================================
    //CONFIG instance
    //rdma has seperate config register, while wdma share with core
    //---------------------------------------
    val u_reg = Module(new NV_soDLA_SDP_reg)
    //glb
...
object NV_soDLA_sdpDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_sdp)
}
