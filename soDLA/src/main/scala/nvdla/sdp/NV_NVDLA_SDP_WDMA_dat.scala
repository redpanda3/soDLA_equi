...
@chiselName
class NV_soDLA_SDP_WDMA_dat(implicit val conf: nvdlaConfig) extends Module {
   val io = IO(new Bundle {
    ...
    val u_in = Module(new NV_soDLA_SDP_WDMA_DAT_in)
    ...
    val u_out = Module(new NV_soDLA_SDP_WDMA_DAT_out)
...
object NV_soDLA_SDP_WDMA_datDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_SDP_WDMA_dat())
}
