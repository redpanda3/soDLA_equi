...
@chiselName
class NV_soDLA_SDP_MRDMA_eg(implicit val conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    val u_cmd = Module(new NV_soDLA_SDP_MRDMA_EG_cmd)
    val u_din = Module(new NV_soDLA_SDP_MRDMA_EG_din)
    val u_dout = Module(new NV_soDLA_SDP_MRDMA_EG_dout)
    //cmd
...
object NV_soDLA_SDP_MRDMA_egDriver extends App {
    implicit val conf: nvdlaConfig = new nvdlaConfig
    chisel3.Driver.execute(args, () => new NV_soDLA_SDP_MRDMA_eg())
}