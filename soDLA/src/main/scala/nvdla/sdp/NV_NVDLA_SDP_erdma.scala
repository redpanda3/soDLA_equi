...
@chiselName
class NV_soDLA_SDP_erdma(implicit conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_clock.nvdla_core_clk){
    ...
    //=======================================
    val u_gate = Module(new NV_soDLA_slcg(1, true))
    ...
    val u_ig = Module(new NV_soDLA_SDP_RDMA_ig)
    ...
    val u_cq = Module(new NV_soDLA_fifo(depth = conf.NVDLA_VMOD_SDP_ERDMA_LATENCY_FIFO_DEPTH, width = 16, distant_wr_req = false, ram_type = 2))
    ...
    val u_eg = Module(new NV_soDLA_SDP_RDMA_eg)
    ...
    val u_lat_fifo = Module(new NV_soDLA_fifo(depth = conf.NVDLA_VMOD_SDP_ERDMA_LATENCY_FIFO_DEPTH, width = conf.NVDLA_DMA_RD_RSP, distant_wr_req = false, ram_type = 2))
    ...
    val u_NV_soDLA_SDP_RDMA_dmaif = Module(new NV_soDLA_SDP_RDMA_dmaif)
    u_NV_soDLA_SDP_RDMA_dmaif.io.nvdla_core_clk := nvdla_gated_clk
    if(conf.NVDLA_SECONDARY_MEMIF_ENABLE){
        io.sdp_e2cvif_rd_req_pd.get <> u_NV_soDLA_SDP_RDMA_dmaif.io.sdp2cvif_rd_req_pd.get
        io.sdp_e2cvif_rd_cdt_lat_fifo_pop.get := u_NV_soDLA_SDP_RDMA_dmaif.io.sdp2cvif_rd_cdt_lat_fifo_pop.get
        u_NV_soDLA_SDP_RDMA_dmaif.io.cvif2sdp_rd_rsp_pd.get <> io.cvif2sdp_e_rd_rsp_pd.get
    }

    io.sdp_e2mcif_rd_req_pd <> u_NV_soDLA_SDP_RDMA_dmaif.io.sdp2mcif_rd_req_pd
    io.sdp_e2mcif_rd_cdt_lat_fifo_pop := u_NV_soDLA_SDP_RDMA_dmaif.io.sdp2mcif_rd_cdt_lat_fifo_pop
    u_NV_soDLA_SDP_RDMA_dmaif.io.mcif2sdp_rd_rsp_pd <> io.mcif2sdp_e_rd_rsp_pd

    u_NV_soDLA_SDP_RDMA_dmaif.io.dma_rd_req_ram_type := io.reg2dp_erdma_ram_type
    u_NV_soDLA_SDP_RDMA_dmaif.io.dma_rd_req_pd <> u_ig.io.dma_rd_req_pd

    u_NV_soDLA_SDP_RDMA_dmaif.io.dma_rd_rsp_ram_type := io.reg2dp_erdma_ram_type
    u_lat_fifo.io.wr_pvld := u_NV_soDLA_SDP_RDMA_dmaif.io.dma_rd_rsp_pd.valid
    u_NV_soDLA_SDP_RDMA_dmaif.io.dma_rd_rsp_pd.ready := u_lat_fifo.io.wr_prdy
    u_lat_fifo.io.wr_pd := u_NV_soDLA_SDP_RDMA_dmaif.io.dma_rd_rsp_pd.bits
    u_NV_soDLA_SDP_RDMA_dmaif.io.dma_rd_cdt_lat_fifo_pop := u_eg.io.dma_rd_cdt_lat_fifo_pop

}}

object NV_soDLA_SDP_erdmaDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_SDP_erdma())
}
