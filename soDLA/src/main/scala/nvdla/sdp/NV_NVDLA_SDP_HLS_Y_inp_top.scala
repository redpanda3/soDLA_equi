...
@chiselName
class NV_soDLA_SDP_HLS_Y_inp_top(implicit conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    val y_int_inp = Array.fill(conf.NVDLA_SDP_EW_THROUGHPUT){Module(new NV_soDLA_SDP_HLS_Y_int_inp)}
...
object NV_soDLA_SDP_HLS_Y_inp_topDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_SDP_HLS_Y_inp_top)
}