...
@chiselName
class NV_soDLA_SDP_rdma(implicit conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_clock.nvdla_core_clk){
    ...
    val u_reg = Module(new NV_soDLA_SDP_RDMA_reg)
    ...
    val u_mrdma = Module(new NV_soDLA_SDP_mrdma)
    ...
    val u_brdma = if(conf.NVDLA_SDP_BS_ENABLE) Some(Module(new NV_soDLA_SDP_brdma)) else None
    ...
    val u_nrdma = if(conf.NVDLA_SDP_BN_ENABLE) Some(Module(new NV_soDLA_SDP_nrdma)) else None
    ...
    val u_erdma = if(conf.NVDLA_SDP_EW_ENABLE) Some(Module(new NV_soDLA_SDP_erdma)) else None
...
object NV_soDLA_SDP_rdmaDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_SDP_rdma())
}
