...
@chiselName
class NV_soDLA_SDP_cmux(implicit val conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    val pipe_p1 = Module(new NV_soDLA_IS_pipe(conf.DP_IN_DW+2))
    ...
    val pipe_p2 = Module(new NV_soDLA_IS_pipe(conf.DP_IN_DW))
...
object NV_soDLA_SDP_cmuxDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_SDP_cmux())
}
