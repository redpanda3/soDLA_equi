...
@chiselName
class NV_soDLA_SDP_wdma(implicit conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_clock.nvdla_core_clk){
    ...
    //=======================================
    val u_gate = Module(new NV_soDLA_slcg(1, false))
    ...
    val u_cmd = Module(new NV_soDLA_SDP_WDMA_cmd)
    ...
    val u_dat = Module(new NV_soDLA_SDP_WDMA_dat)
    ...
    val u_dmaif_wr = Module(new NV_soDLA_DMAIF_wr(conf.NVDLA_DMA_WR_REQ))
    ...
    val u_intr = Module(new NV_soDLA_SDP_WDMA_intr)
...
object NV_soDLA_SDP_wdmaDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_SDP_wdma())
}
