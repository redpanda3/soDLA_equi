...
@chiselName
class NV_soDLA_SDP_mrdma(implicit conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_clock.nvdla_core_clk){
    ...
    //---------------------------------------
    val u_ig = Module(new NV_soDLA_SDP_MRDMA_ig)
    ...
    val cq_width = 14
    val u_cq = Module(new NV_soDLA_fifo(depth = cq_depth, width = cq_width, distant_wr_req = false, ram_type = 2))
    ...
    val u_eg = Module(new NV_soDLA_SDP_MRDMA_eg)
    ...
    if(conf.NVDLA_SECONDARY_MEMIF_ENABLE){
        ...
        io.sdp2cvif_rd_req_pd.get <> u_NV_soDLA_SDP_RDMA_dmaif.io.sdp2cvif_rd_req_pd.get
        io.sdp2cvif_rd_cdt_lat_fifo_pop.get := u_NV_soDLA_SDP_RDMA_dmaif.io.sdp2cvif_rd_cdt_lat_fifo_pop.get
        u_NV_soDLA_SDP_RDMA_dmaif.io.cvif2sdp_rd_rsp_pd.get <> io.cvif2sdp_rd_rsp_pd.get
    }

    io.sdp2mcif_rd_req_pd <> u_NV_soDLA_SDP_RDMA_dmaif.io.sdp2mcif_rd_req_pd
    io.sdp2mcif_rd_cdt_lat_fifo_pop := u_NV_soDLA_SDP_RDMA_dmaif.io.sdp2mcif_rd_cdt_lat_fifo_pop
    u_NV_soDLA_SDP_RDMA_dmaif.io.mcif2sdp_rd_rsp_pd <> io.mcif2sdp_rd_rsp_pd

    u_NV_soDLA_SDP_RDMA_dmaif.io.dma_rd_req_pd <> u_ig.io.dma_rd_req_pd
    u_NV_soDLA_SDP_RDMA_dmaif.io.dma_rd_req_ram_type := u_ig.io.dma_rd_req_ram_type

    u_NV_soDLA_SDP_RDMA_dmaif.io.dma_rd_rsp_ram_type := u_eg.io.dma_rd_rsp_ram_type
    u_eg.io.dma_rd_rsp_pd <> u_NV_soDLA_SDP_RDMA_dmaif.io.dma_rd_rsp_pd
    u_NV_soDLA_SDP_RDMA_dmaif.io.dma_rd_cdt_lat_fifo_pop := u_eg.io.dma_rd_cdt_lat_fifo_pop

}}

object NV_soDLA_SDP_mrdmaDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_SDP_mrdma())
}
