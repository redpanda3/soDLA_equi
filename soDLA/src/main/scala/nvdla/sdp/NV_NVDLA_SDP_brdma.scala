...
@chiselName
class NV_soDLA_SDP_brdma(implicit conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_clock.nvdla_core_clk){
    ...
    val u_lat_fifo = Module(new NV_soDLA_fifo(depth = conf.NVDLA_VMOD_SDP_BRDMA_LATENCY_FIFO_DEPTH, width = conf.NVDLA_DMA_RD_RSP, distant_wr_req = false, ram_type = 2))
    ...
    if(conf.NVDLA_SECONDARY_MEMIF_ENABLE){
        io.sdp_b2cvif_rd_req_pd.get <> u_NV_soDLA_SDP_RDMA_dmaif.io.sdp2cvif_rd_req_pd.get
        io.sdp_b2cvif_rd_cdt_lat_fifo_pop.get := u_NV_soDLA_SDP_RDMA_dmaif.io.sdp2cvif_rd_cdt_lat_fifo_pop.get
        u_NV_soDLA_SDP_RDMA_dmaif.io.cvif2sdp_rd_rsp_pd.get <> io.cvif2sdp_b_rd_rsp_pd.get
    }
    io.sdp_b2mcif_rd_req_pd <> u_NV_soDLA_SDP_RDMA_dmaif.io.sdp2mcif_rd_req_pd
    io.sdp_b2mcif_rd_cdt_lat_fifo_pop := u_NV_soDLA_SDP_RDMA_dmaif.io.sdp2mcif_rd_cdt_lat_fifo_pop
    u_NV_soDLA_SDP_RDMA_dmaif.io.mcif2sdp_rd_rsp_pd <> io.mcif2sdp_b_rd_rsp_pd

    u_NV_soDLA_SDP_RDMA_dmaif.io.dma_rd_req_ram_type := io.reg2dp_brdma_ram_type
    u_NV_soDLA_SDP_RDMA_dmaif.io.dma_rd_req_pd <> u_ig.io.dma_rd_req_pd

    u_NV_soDLA_SDP_RDMA_dmaif.io.dma_rd_rsp_ram_type := io.reg2dp_brdma_ram_type
...
object NV_soDLA_SDP_brdmaDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_SDP_brdma())
}
