...
@chiselName
class NV_soDLA_SDP_WDMA_DAT_in(implicit val conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    val u_dfifo = Array.fill(4){Module(new NV_soDLA_IS_pipe(conf.AM_DW))}
    for(i <- 0 to 3){
...
object NV_soDLA_SDP_WDMA_DAT_inDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_SDP_WDMA_DAT_in())
}
