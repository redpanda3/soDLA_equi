...
@chiselName
class NV_soDLA_SDP_RDMA_dmaif(implicit conf: nvdlaConfig) extends Module {
...
//==============
    // rd Channel: Request
    val NV_soDLA_SDP_RDMA_rdreq = Module{new NV_soDLA_DMAIF_rdreq(conf.NVDLA_DMA_RD_REQ)}
    NV_soDLA_SDP_RDMA_rdreq.io.nvdla_core_clk := io.nvdla_core_clk
    if(conf.NVDLA_SECONDARY_MEMIF_ENABLE){
        io.sdp2cvif_rd_req_pd.get <> NV_soDLA_SDP_RDMA_rdreq.io.cvif_rd_req_pd.get
    }
    io.sdp2mcif_rd_req_pd <> NV_soDLA_SDP_RDMA_rdreq.io.mcif_rd_req_pd
    NV_soDLA_SDP_RDMA_rdreq.io.dmaif_rd_req_pd <> io.dma_rd_req_pd
    NV_soDLA_SDP_RDMA_rdreq.io.reg2dp_src_ram_type := io.dma_rd_req_ram_type

    // rd Channel: Response
    val NV_soDLA_SDP_RDMA_rdrsp = Module{new NV_soDLA_DMAIF_rdrsp(conf.NVDLA_DMA_RD_RSP)}
    NV_soDLA_SDP_RDMA_rdrsp.io.nvdla_core_clk := io.nvdla_core_clk
    if(conf.NVDLA_SECONDARY_MEMIF_ENABLE){
        NV_soDLA_SDP_RDMA_rdrsp.io.cvif_rd_rsp_pd.get <> io.cvif2sdp_rd_rsp_pd.get
    }
    NV_soDLA_SDP_RDMA_rdrsp.io.mcif_rd_rsp_pd <> io.mcif2sdp_rd_rsp_pd

    io.dma_rd_rsp_pd <> NV_soDLA_SDP_RDMA_rdrsp.io.dmaif_rd_rsp_pd
...
object NV_soDLA_SDP_RDMA_dmaifDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_SDP_RDMA_dmaif())
}
