...
@chiselName
class NV_soDLA_SDP_WDMA_unpack extends Module {
   val IW = 256
   val IHW = IW/2
   val OW = 256
   val RATIO = OW/IW
...
object NV_soDLA_SDP_WDMA_unpackDriver extends App {
  chisel3.Driver.execute(args, () => new NV_soDLA_SDP_WDMA_unpack())
}