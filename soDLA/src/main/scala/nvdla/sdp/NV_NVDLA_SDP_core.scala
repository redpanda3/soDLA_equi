...
@chiselName
class NV_soDLA_SDP_core(implicit val conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_clock.nvdla_core_clk){   
    ...
    val u_gate = Array.fill(3){Module(new NV_soDLA_slcg(1, false))}
    for(i <- 0 to 2){
    ...
    val u_dpin_pack = Module(new NV_soDLA_SDP_RDMA_pack(conf.DP_DIN_DW, conf.DP_IN_DW, 2))
    ...
    val u_bs_mul_pack = if(conf.NVDLA_SDP_BS_ENABLE) Some(Module(new NV_soDLA_SDP_RDMA_pack(conf.AM_DW2, conf.BS_OP_DW, 1))) else None
    val u_bs_alu_pack = if(conf.NVDLA_SDP_BS_ENABLE) Some(Module(new NV_soDLA_SDP_RDMA_pack(conf.AM_DW2, conf.BS_OP_DW, 1))) else None
    if(conf.NVDLA_SDP_BS_ENABLE){
    ...
    val u_bn_mul_pack = if(conf.NVDLA_SDP_BN_ENABLE) Some(Module(new NV_soDLA_SDP_RDMA_pack(conf.AM_DW2, conf.BN_OP_DW, 1))) else None
    val u_bn_alu_pack = if(conf.NVDLA_SDP_BN_ENABLE) Some(Module(new NV_soDLA_SDP_RDMA_pack(conf.AM_DW2, conf.BN_OP_DW, 1))) else None
    if(conf.NVDLA_SDP_BN_ENABLE){
    ...
    val u_ew_mul_pack = if(conf.NVDLA_SDP_EW_ENABLE) Some(Module(new NV_soDLA_SDP_RDMA_pack(conf.AM_DW2, conf.EW_OP_DW, 1))) else None
    val u_ew_alu_pack = if(conf.NVDLA_SDP_EW_ENABLE) Some(Module(new NV_soDLA_SDP_RDMA_pack(conf.AM_DW2, conf.EW_OP_DW, 1))) else None
    if(conf.NVDLA_SDP_EW_ENABLE){
    ...
    val u_NV_soDLA_SDP_cmux = Module(new NV_soDLA_SDP_cmux)
    u_NV_soDLA_SDP_cmux.io.nvdla_core_clk := io.nvdla_clock.nvdla_core_clk

    u_NV_soDLA_SDP_cmux.io.cacc2sdp_pd <> io.cacc2sdp_pd
    u_NV_soDLA_SDP_cmux.io.sdp_mrdma2cmux_pd <> u_dpin_pack.io.out

    val sdp_cmux2dp_valid = u_NV_soDLA_SDP_cmux.io.sdp_cmux2dp_pd.valid
    u_NV_soDLA_SDP_cmux.io.sdp_cmux2dp_pd.ready := sdp_cmux2dp_ready
    val sdp_cmux2dp_pd = u_NV_soDLA_SDP_cmux.io.sdp_cmux2dp_pd.bits

    u_NV_soDLA_SDP_cmux.io.reg2dp_flying_mode := io.reg2dp_flying_mode
    u_NV_soDLA_SDP_cmux.io.reg2dp_nan_to_zero := io.reg2dp_nan_to_zero
    u_NV_soDLA_SDP_cmux.io.reg2dp_proc_precision := io.reg2dp_proc_precision
    u_NV_soDLA_SDP_cmux.io.op_en_load := op_en_load
    ...
    val u_bs_dppack = if(conf.NVDLA_SDP_BS_ENABLE) Some(Module(new NV_soDLA_SDP_CORE_pack(conf.DP_IN_DW, conf.BS_IN_DW))) else None
    val u_bs = if(conf.NVDLA_SDP_BS_ENABLE) Some(Module(new NV_soDLA_SDP_HLS_x_int(conf.NVDLA_SDP_BS_THROUGHPUT))) else None
    val u_bs_dpunpack = if(conf.NVDLA_SDP_BS_ENABLE) Some(Module(new NV_soDLA_SDP_CORE_unpack(conf.BS_OUT_DW, conf.BS_DOUT_DW))) else None
    ...
    val u_bn_dppack = if(conf.NVDLA_SDP_BN_ENABLE) Some(Module(new NV_soDLA_SDP_CORE_pack(conf.BN_DIN_DW, conf.BN_IN_DW))) else None
    val u_bn = if(conf.NVDLA_SDP_BN_ENABLE) Some(Module(new NV_soDLA_SDP_HLS_x_int(conf.NVDLA_SDP_BN_THROUGHPUT))) else None
    val u_bn_dpunpack = if(conf.NVDLA_SDP_BN_ENABLE) Some(Module(new NV_soDLA_SDP_CORE_unpack(conf.BN_OUT_DW, conf.BN_DOUT_DW))) else None
    if(conf.NVDLA_SDP_BN_ENABLE){
    ...
    val u_ew_dppack = if(conf.NVDLA_SDP_EW_ENABLE) Some(Module(new NV_soDLA_SDP_CORE_pack(conf.EW_DIN_DW, conf.EW_IN_DW))) else None
    val u_ew = if(conf.NVDLA_SDP_EW_ENABLE) Some(Module(new NV_soDLA_SDP_CORE_y)) else None
    val u_ew_dpunpack = if(conf.NVDLA_SDP_EW_ENABLE) Some(Module(new NV_soDLA_SDP_CORE_unpack(conf.EW_OUT_DW, conf.EW_DOUT_DW))) else None
    if(conf.NVDLA_SDP_EW_ENABLE){
    ...
    val u_c = Module(new NV_soDLA_SDP_HLS_c)
    ...
    //only int8 or int16. If support both, use NV_soDLA_SDP_WDMA_unpack
    val u_dpout_unpack = Module(new NV_soDLA_SDP_CORE_unpack(conf.DP_OUT_DW, conf.DP_DOUT_DW))
    ...
    val pipe_p11 = Module{new NV_soDLA_IS_pipe(conf.DP_OUT_DW)}
...
object NV_soDLA_SDP_coreDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_SDP_core)
}
