...
@chiselName
class NV_soDLA_SDP_RDMA_EG_ro(implicit val conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    val u_rod = Array.fill(4){Module(new NV_soDLA_IS_pipe(conf.AM_DW))}
    for(i <- 0 to 3){
    ...
    val u_roc = Module(new NV_soDLA_fifo(depth = 4, width = 2, ram_type = 0, distant_wr_req = false))
    ...
    val pipe_p1 = Module{new NV_soDLA_IS_pipe(conf.AM_DW2+1)}
...
object NV_soDLA_SDP_RDMA_EG_roDriver extends App {
    implicit val conf: nvdlaConfig = new nvdlaConfig
    chisel3.Driver.execute(args, () => new NV_soDLA_SDP_RDMA_EG_ro())
}
