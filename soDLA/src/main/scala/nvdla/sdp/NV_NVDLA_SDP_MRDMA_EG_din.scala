...
@chiselName
class NV_soDLA_SDP_MRDMA_EG_din(implicit val conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    val u_lat_fifo = Module(new NV_soDLA_fifo(depth = conf.NVDLA_VMOD_SDP_MRDMA_LATENCY_FIFO_DEPTH, width = conf.NVDLA_DMA_RD_RSP, ram_type = 2, distant_wr_req = false))
    ...
    val u_rdma_unpack = Module(new NV_soDLA_SDP_RDMA_unpack)
    ...
    val u_pfifo =  Array.fill(4){Module(new NV_soDLA_IS_pipe(conf.AM_DW))}
    for(i <- 0 to 3){
...
object NV_soDLA_SDP_MRDMA_EG_dinDriver extends App {
    implicit val conf: nvdlaConfig = new nvdlaConfig
    chisel3.Driver.execute(args, () => new NV_soDLA_SDP_MRDMA_EG_din())
}