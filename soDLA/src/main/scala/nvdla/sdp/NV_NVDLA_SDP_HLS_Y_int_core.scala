...
@chiselName
class NV_soDLA_SDP_HLS_Y_int_core(implicit conf: nvdlaConfig) extends Module{
...
withClock(io.nvdla_core_clk){
    ...
    val u_sdp_y_core_mul = Array.fill(conf.NVDLA_SDP_EW_THROUGHPUT){Module{new NV_soDLA_SDP_HLS_Y_int_mul}}
    val u_sdp_y_core_alu = Array.fill(conf.NVDLA_SDP_EW_THROUGHPUT){Module{new NV_soDLA_SDP_HLS_Y_int_alu}}

    for(i<- 0 to conf.NVDLA_SDP_EW_THROUGHPUT-1){

    //: NV_soDLA_SDP_HLS_Y_int_mul
...
object NV_soDLA_SDP_HLS_Y_int_coreDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_SDP_HLS_Y_int_core)
}