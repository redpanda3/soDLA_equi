...
@chiselName
class NV_soDLA_SDP_WDMA_cmd(implicit val conf: nvdlaConfig) extends Module {
...
// //==============
    ...
    val u_sfifo = Module{new NV_soDLA_fifo(depth = 4, width = 15, distant_wr_req = false, ram_type = 0)}
    ...
    val u_dfifo = Module{new NV_soDLA_fifo(depth = 4, width = 44, distant_wr_req = false, ram_type = 0)}
...
object NV_soDLA_SDP_WDMA_cmdDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_SDP_WDMA_cmd())
}
