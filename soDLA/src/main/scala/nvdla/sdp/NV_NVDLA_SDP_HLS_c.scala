...
@chiselName
class NV_soDLA_SDP_HLS_c(implicit conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    val c_int = Array.fill(conf.NVDLA_SDP_MAX_THROUGHPUT){Module(new NV_soDLA_SDP_HLS_C_int)}
...
object NV_soDLA_SDP_HLS_cDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_SDP_HLS_c)
}