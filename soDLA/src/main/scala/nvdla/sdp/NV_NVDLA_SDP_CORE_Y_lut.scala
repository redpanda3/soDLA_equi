...
class NV_soDLA_SDP_CORE_Y_lut(implicit val conf: nvdlaConfig) extends Module {
...
val pipe_p1 = Module(new NV_soDLA_IS_pipe(conf.EW_IDX_OUT_DW))
...
val u_dat = Module(new NV_soDLA_fifo(depth = 2, width = 32*conf.NVDLA_SDP_EW_THROUGHPUT, ram_type = 0, distant_wr_req = false))
...
val u_cmd = Module{new NV_soDLA_fifo(depth = 2, width = 70*conf.NVDLA_SDP_EW_THROUGHPUT, ram_type = 0, distant_wr_req = false)}
...
val pipe_p2 = Module(new NV_soDLA_IS_pipe(conf.EW_LUT_OUT_DW))
...
object NV_soDLA_SDP_CORE_Y_lutDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_SDP_CORE_Y_lut)
}
