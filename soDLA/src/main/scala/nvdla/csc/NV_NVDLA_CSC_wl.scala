...
@chiselName
class NV_soDLA_CSC_wlIO(implicit conf: nvdlaConfig) extends Bundle{
...
class NV_soDLA_CSC_wl(implicit val conf: nvdlaConfig) extends Module {
    val io = IO(new NV_soDLA_CSC_wlIO)
    //     
    //          ┌─┐       ┌─┐
    //       ┌──┘ ┴───────┘ ┴──┐
    //       │                 │
...
withClock(io.nvdla_core_clk){
    ...
    val u_dec = Module(new NV_soDLA_CSC_WL_dec)
...
object NV_soDLA_CSC_wlDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_CSC_wl)
}
