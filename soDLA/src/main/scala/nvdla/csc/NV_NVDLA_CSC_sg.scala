...
@chiselName
class NV_soDLA_CSC_sg(implicit val conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    val u_dat_fifo = Module(new NV_soDLA_fifo(depth = 4, width = 33,
    ...
    val u_wt_fifo = Module(new NV_soDLA_fifo(depth = 4, width = 20,
                        ram_type = 2, distant_wr_req = true,
                        io_wr_empty = true ))
...
object NV_soDLA_CSC_sgDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_CSC_sg())
}
