...
@chiselName
class NV_soDLA_MCIF_READ_eg(implicit conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk) {
    ...
    val pipe_pr = Module(new NV_soDLA_IS_pipe(conf.NVDLA_PRIMARY_MEMIF_WIDTH + 4))
    ...
    val lat_fifo = Array.fill(conf.RDMA_NUM){Module(new NV_soDLA_fifo(depth = 4,width = conf.NVDLA_PRIMARY_MEMIF_WIDTH, ram_type = 0))}
    ...
    val pipe_out = Array.fill(conf.RDMA_NUM){Module(new NV_soDLA_IS_pipe(conf.NVDLA_DMA_RD_RSP)) }
    for(i<-0 until conf.RDMA_NUM) {
...
object NV_soDLA_MCIF_READ_egDriver extends App {
    implicit val conf: nvdlaConfig = new nvdlaConfig
    chisel3.Driver.execute(args, () => new NV_soDLA_MCIF_READ_eg())
}
