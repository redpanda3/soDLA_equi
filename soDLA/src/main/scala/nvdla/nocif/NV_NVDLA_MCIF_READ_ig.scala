...
@chiselName
class NV_soDLA_MCIF_READ_ig (implicit conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    //---------------------read_bpt inst--------------------------------//
    val u_bpt = Array.fill(conf.RDMA_NUM){Module(new NV_soDLA_MCIF_READ_IG_bpt)}
    ...
    //arb
    val u_arb = Module(new NV_soDLA_MCIF_READ_IG_arb)
    ...
    val u_cvt = Module(new NV_soDLA_XXIF_READ_IG_cvt(cq_enabled = false))
...
object NV_soDLA_MCIF_READ_igDriver extends App {
    implicit val conf: nvdlaConfig = new nvdlaConfig
    chisel3.Driver.execute(args, () => new NV_soDLA_MCIF_READ_ig())
}
