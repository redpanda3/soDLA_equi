...
@chiselName
class NV_soDLA_MCIF_WRITE_IG_bpt(implicit conf:nvdlaConfig)  extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    val pipe_p1 = Module(new NV_soDLA_IS_pipe(conf.NVDLA_DMA_WR_REQ))
    pipe_p1.io.clk := io.nvdla_core_clk
    pipe_p1.io.vi := io.dma2bpt_req_pd.valid
    io.dma2bpt_req_pd.ready := pipe_p1.io.ro
    pipe_p1.io.di := io.dma2bpt_req_pd.bits

    val pipe_p2 = Module(new NV_soDLA_IS_pipe(conf.NVDLA_DMA_WR_REQ))
    ...
    val pipe_p3 = Module(new NV_soDLA_BC_pipe(conf.NVDLA_DMA_WR_CMD))
    ...
    val u_dfifo = Module(new NV_soDLA_IS_pipe(conf.NVDLA_MEMIF_WIDTH))
 ...
 object NV_soDLA_MCIF_WRITE_IG_bptDriver extends App {
     implicit val conf: nvdlaConfig = new nvdlaConfig
     chisel3.Driver.execute(args, () => new NV_soDLA_MCIF_WRITE_IG_bpt())
 }