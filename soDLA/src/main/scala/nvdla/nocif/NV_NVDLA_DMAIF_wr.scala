...
@chiselName
class NV_soDLA_DMAIF_wr(DMABW: Int)(implicit conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    val is_pipe1 = Module{new NV_soDLA_IS_pipe(DMABW+1)}
    ...
    if(conf.NVDLA_SECONDARY_MEMIF_ENABLE){
        val is_pipe2 = Module{new NV_soDLA_IS_pipe(DMABW+1)}
