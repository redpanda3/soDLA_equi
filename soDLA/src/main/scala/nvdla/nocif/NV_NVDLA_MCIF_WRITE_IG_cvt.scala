...
@chiselName
class NV_soDLA_MCIF_WRITE_IG_cvt(implicit conf:nvdlaConfig)  extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    val pipe_p1 = Module(new NV_soDLA_BC_pipe(conf.NVDLA_DMA_WR_IG_PW))
    ...
    val pipe_p2 = Module(new NV_soDLA_BC_pipe(conf.NVDLA_PRIMARY_MEMIF_WIDTH + conf.NVDLA_DMA_MASK_BIT))
    ...
    val pipe_p3 = Module(new NV_soDLA_IS_pipe(conf.NVDLA_MEM_ADDRESS_WIDTH+6))
    ...
    val pipe_p4 = Module(new NV_soDLA_IS_pipe(conf.NVDLA_PRIMARY_MEMIF_WIDTH+conf.NVDLA_PRIMARY_MEMIF_STRB+1))
...
object NV_soDLA_MCIF_WRITE_IG_cvtDriver extends App {
    implicit val conf: nvdlaConfig = new nvdlaConfig
    chisel3.Driver.execute(args, () => new NV_soDLA_MCIF_WRITE_IG_cvt)
}