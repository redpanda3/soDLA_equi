...
@chiselName
class NV_soDLA_MCIF_WRITE_IG_arb(implicit conf:nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){   
    ...
    val u_pipe = Array.fill(conf.WDMA_NUM){Module(new NV_soDLA_BC_pipe(conf.NVDLA_DMA_WR_IG_PW))}
    val u_dfifo = Array.fill(conf.WDMA_NUM){Module(new NV_soDLA_fifo(depth = 4, width = conf.NVDLA_DMA_WR_REQ-1, ram_type = 0, io_wr_count = true))}
    ...
    val u_write_ig_arb = Module(new NV_soDLA_arb(n = conf.WDMA_MAX_NUM, wt_width = 8, io_gnt_busy = true))
 ...
 object NV_soDLA_MCIF_WRITE_IG_arbDriver extends App {
     implicit val conf: nvdlaConfig = new nvdlaConfig
     chisel3.Driver.execute(args, () => new NV_soDLA_MCIF_WRITE_IG_arb())
 }
