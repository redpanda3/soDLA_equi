...
@chiselName
class NV_soDLA_nocif(implicit conf: nvdlaConfig) extends Module {
...
withReset(!io.nvdla_core_rstn){
    ...
    val u_mcif = Module(new NV_soDLA_mcif)
    ...
    val u_cvif = if(conf.NVDLA_SECONDARY_MEMIF_ENABLE) Some(Module(new NV_soDLA_mcif)) else None
...
object NV_soDLA_nocifDriver extends App {
    implicit val conf: nvdlaConfig = new nvdlaConfig
    chisel3.Driver.execute(args, () => new NV_soDLA_nocif())
}
