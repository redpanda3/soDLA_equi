...
@chiselName
 class NV_soDLA_MCIF_READ_IG_arb(implicit conf:nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    val pipe_p = Array.fill(conf.RDMA_NUM){Module(new NV_soDLA_IS_pipe(conf.NVDLA_DMA_RD_IG_PW))}
    ...
    val u_read_ig_arb = Module(new NV_soDLA_arb(n = conf.RDMA_MAX_NUM, wt_width = 8, io_gnt_busy = true))
    ...
    val pipe_out = Module(new NV_soDLA_IS_pipe(conf.NVDLA_DMA_RD_IG_PW))
 ...
 object NV_soDLA_MCIF_READ_IG_arbDriver extends App {
     implicit val conf: nvdlaConfig = new nvdlaConfig
     chisel3.Driver.execute(args, () => new NV_soDLA_MCIF_READ_IG_arb())
 }
