...
@chiselName
class NV_soDLA_CMAC_reg(implicit val conf: nvdlaConfig, useRealClock:Boolean=false) extends Module {
...
withClock(internal_clock){
    ...
    val u_dual_reg_d0 = Module(new NV_soDLA_CMAC_REG_dual)
    ...
    val u_dual_reg_d1 = Module(new NV_soDLA_CMAC_REG_dual)
