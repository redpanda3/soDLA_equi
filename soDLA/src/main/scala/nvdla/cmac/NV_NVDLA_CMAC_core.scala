...
@chiselName
class NV_soDLA_CMAC_core(implicit val conf: nvdlaConfig) extends Module {
...
//             └──┴──┘       └──┴──┘
    ...
    //==========================================================
    val u_rt_in = Module(new NV_soDLA_CMAC_CORE_rt_in(useRealClock = true))
    ...
    //==========================================================
    val u_active = Module(new NV_soDLA_CMAC_CORE_active(useRealClock = true))
    ...
    // MAC CELLs
    //==========================================================
    val u_mac = Array.fill(conf.CMAC_ATOMK_HALF){Module(new NV_soDLA_CMAC_CORE_mac(useRealClock = true))}
    val u_rt_out = Module(new NV_soDLA_CMAC_CORE_rt_out(useRealClock = true))  // use seq
    ...
    // SLCG groups
    //==========================================================
    val u_slcg_op = Array.fill(conf.CMAC_ATOMK_HALF+3){Module(new NV_soDLA_slcg(1, false))}
...
object NV_soDLA_CMAC_coreDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_CMAC_core)
}
