...
@chiselName
class NV_soDLA_CDP_DP_syncfifo(implicit val conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    val pipe_0 = Module(new NV_soDLA_BC_pipe(conf.NVDLA_CDP_THROUGHPUT * conf.NVDLA_CDP_ICVTO_BWPE))
    pipe_0.io.clk := io.nvdla_core_clk
    pipe_0.io.vi := data_vld
    data_rdy := pipe_0.io.ro
    pipe_0.io.di := data_pd

    val pipe_1 = Module(new NV_soDLA_BC_pipe(7))
    ...
    //////////////////////////////////////////////
    //////////////////////////////////////////////
    val u_data_sync_fifo = Module{new NV_soDLA_fifo(
                            depth = 80, 
    ...
    ///////////////////////////////////////////
    val u_info_sync_fifo = Module{new NV_soDLA_fifo(
                            depth = 80, 
                            width = 17,
                            ram_type = 2, 
                            distant_wr_req = false)}
    ...
    ///////////////////////////////////////////
    val u_sumpd_sync_fifo = Module{new NV_soDLA_fifo(
                            depth = 80, 
...
object NV_soDLA_CDP_DP_syncfifoDriver extends App {
    implicit val conf: nvdlaConfig = new nvdlaConfig
    chisel3.Driver.execute(args, () => new NV_soDLA_CDP_DP_syncfifo())
}
