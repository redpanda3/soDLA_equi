...
@chiselName
class NV_soDLA_CDP_DP_intp(implicit val conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    val pipe = Module(new NV_soDLA_IS_pipe(conf.NVDLA_CDP_THROUGHPUT*103))
    ...
    val u_interp_x = Array.fill(conf.NVDLA_CDP_THROUGHPUT){Module(new NV_soDLA_CDP_DP_INTP_unit)}
    for(i <- 0 to (conf.NVDLA_CDP_THROUGHPUT-1)){
    ...
    val u_intpinfo_sync_fifo = Module(new NV_soDLA_fifo(20, conf.NVDLA_CDP_THROUGHPUT*4))
    ...
    ////////////////////////////////////////////////
    val ip2mul_pipe = Array.fill(conf.NVDLA_CDP_THROUGHPUT){Module(new NV_soDLA_IS_pipe(17))}
...
object NV_soDLA_CDP_DP_intpDriver extends App {
    implicit val conf: nvdlaConfig = new nvdlaConfig
    chisel3.Driver.execute(args, () => new NV_soDLA_CDP_DP_intp())
}
