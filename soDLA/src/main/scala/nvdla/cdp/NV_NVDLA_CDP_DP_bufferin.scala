...
@chiselName
class NV_soDLA_CDP_DP_bufferin(implicit val conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    val pipe_p0 = Module(new NV_soDLA_IS_pipe(conf.NVDLA_CDP_THROUGHPUT*conf.NVDLA_CDP_ICVTO_BWPE+17))
    ...
    //==============
    val u_unpack = Module(new NV_soDLA_CDP_DP_BUFFERIN_unpack)
    ...
    //Two cycle delay
    ///////////////////
    val u_delay = Module(new NV_soDLA_CDP_DP_BUFFERIN_two_cycle_delay)
    ...
    val pipe_p2 = Module(new NV_soDLA_IS_pipe((conf.NVDLA_CDP_THROUGHPUT+8)*conf.NVDLA_CDP_ICVTO_BWPE+17))
...
object NV_soDLA_CDP_DP_bufferinDriver extends App {
    implicit val conf: nvdlaConfig = new nvdlaConfig
    chisel3.Driver.execute(args, () => new NV_soDLA_CDP_DP_bufferin())
}
