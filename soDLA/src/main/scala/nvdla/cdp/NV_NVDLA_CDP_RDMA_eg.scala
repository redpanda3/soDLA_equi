...
@chiselName
class NV_soDLA_CDP_RDMA_eg(implicit val conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    val NV_soDLA_CDP_RDMA_rdrsp = Module(new NV_soDLA_DMAIF_rdrsp(conf.NVDLA_CDP_MEM_RD_RSP))
    NV_soDLA_CDP_RDMA_rdrsp.io.nvdla_core_clk := io.nvdla_core_clk

    NV_soDLA_CDP_RDMA_rdrsp.io.mcif_rd_rsp_pd <> io.mcif2cdp_rd_rsp_pd

    if(conf.NVDLA_SECONDARY_MEMIF_ENABLE){
        NV_soDLA_CDP_RDMA_rdrsp.io.cvif_rd_rsp_pd.get <> io.cvif2cdp_rd_rsp_pd.get
    }
    ...
    u_lat_fifo.io.wr_pvld := NV_soDLA_CDP_RDMA_rdrsp.io.dmaif_rd_rsp_pd.valid
    NV_soDLA_CDP_RDMA_rdrsp.io.dmaif_rd_rsp_pd.ready := u_lat_fifo.io.wr_prdy
    u_lat_fifo.io.wr_pd := NV_soDLA_CDP_RDMA_rdrsp.io.dmaif_rd_rsp_pd.bits
    ...
    val u_ro_fifo = Array.fill(conf.TOTAL_CDP_NUM){Module(new NV_soDLA_fifo(
...
ele_in_channel(conf.ATMMBW-1, conf.CDP_TPBW)
    ...
    val pipe_p1 = Module(new NV_soDLA_IS_pipe(conf.NVDLA_CDP_THROUGHPUT*conf.NVDLA_BPE+27))
...
object NV_soDLA_CDP_RDMA_egDriver extends App {
    implicit val conf: nvdlaConfig = new nvdlaConfig
    chisel3.Driver.execute(args, () => new NV_soDLA_CDP_RDMA_eg())
}
