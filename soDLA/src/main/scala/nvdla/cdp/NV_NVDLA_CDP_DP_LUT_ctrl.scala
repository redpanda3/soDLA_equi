...
@chiselName
class NV_soDLA_CDP_DP_LUT_ctrl(implicit val conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    val u_LUT_CTRL_unit = Array.fill(conf.NVDLA_CDP_THROUGHPUT){Module(new NV_soDLA_CDP_DP_LUT_CTRL_unit)}
    for(i <- 0 to (conf.NVDLA_CDP_THROUGHPUT-1)){
...
object NV_soDLA_CDP_DP_LUT_ctrlDriver extends App {
    implicit val conf: nvdlaConfig = new nvdlaConfig
    chisel3.Driver.execute(args, () => new NV_soDLA_CDP_DP_LUT_ctrl())
}
