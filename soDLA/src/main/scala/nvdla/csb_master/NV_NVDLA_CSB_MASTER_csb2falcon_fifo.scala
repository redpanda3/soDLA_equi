...
@chiselName
class NV_soDLA_CSB_MASTER_csb2falcon_fifo(implicit val conf: nvdlaConfig)  extends Module {
...
//
    ...
    val wr_pushing_gray = Module(new NV_soDLA_CSB_MASTER_csb2falcon_fifo_gray_cntr_strict)
    ...
    val rd_pushing_gray = Module(new NV_soDLA_CSB_MASTER_csb2falcon_fifo_gray_cntr)
    ...
    val rd_popping_gray = Module(new NV_soDLA_CSB_MASTER_csb2falcon_fifo_gray_cntr_strict)
    if(conf.NV_FPGA_FIFOGEN){
    ...
    val wr_popping_gray = Module(new NV_soDLA_CSB_MASTER_csb2falcon_fifo_gray_cntr)
...
class NV_soDLA_CSB_MASTER_csb2falcon_fifo_gray_cntr_strict(implicit val conf: nvdlaConfig) extends Module {
  val io = IO(new Bundle{
...
class NV_soDLA_CSB_MASTER_csb2falcon_fifo_gray_cntr extends Module {
  val io = IO(new Bundle{
      val clk = Input(Clock())
      val inc = Input(Bool())
...
object NV_soDLA_CSB_MASTER_csb2falcon_fifoDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_CSB_MASTER_csb2falcon_fifo())
}
