...
@chiselName
class NV_soDLA_PDP_rdma(implicit val conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_clock.nvdla_core_clk){
    ...
    val u_reg = Module(new NV_soDLA_PDP_RDMA_reg)
    val u_ig = Module(new NV_soDLA_PDP_RDMA_ig)
    val u_cq = Module(new NV_soDLA_fifo(
                        depth = 256, 
                        width = 18,
                        ram_type = 0,
                        distant_wr_req = false))
    val u_eg = Module(new NV_soDLA_PDP_RDMA_eg)
    //=======================================
    //        SLCG gen unit
    //---------------------------------------
...
object NV_soDLA_PDP_rdmaDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_PDP_rdma())
}