...
@chiselName
class NV_soDLA_PDP_CORE_CAL2D_pad_v_and_div_kernel(implicit val conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    val u_pad = Module(new NV_soDLA_VEC_padder(vector_len = conf.NVDLA_PDP_THROUGHPUT, data_width = conf.NVDLA_BPE+7))
    ...
    val u_div_kwidth = Module(new NV_soDLA_VEC_DIV_kernel(vector_len = conf.NVDLA_PDP_THROUGHPUT, 
    ...
    val u_div_kheight = Module(new NV_soDLA_VEC_DIV_kernel(vector_len = conf.NVDLA_PDP_THROUGHPUT, 
...
class NV_soDLA_PDP_CORE_CAL2D_calculate_real_pooling_size(implicit val conf: nvdlaConfig) extends Module {
    val io = IO(new Bundle {
        //clk
        val nvdla_core_clk = Input(Clock())
...
class NV_soDLA_PDP_CORE_CAL2D_bank_merge_num(implicit val conf: nvdlaConfig) extends Module {
    val io = IO(new Bundle {
        val nvdla_core_clk = Input(Clock())
