...
@chiselName
class NV_soDLA_PDP_CORE_cal2dIO(implicit conf: nvdlaConfig) extends Bundle{
    //clk
...
class NV_soDLA_PDP_CORE_cal2d(implicit val conf: nvdlaConfig) extends Module {
    val io = IO(new NV_soDLA_PDP_CORE_cal2dIO)
...
withClock(io.nvdla_core_clk){
    ...
    val u_bank_merge_num = Module(new NV_soDLA_PDP_CORE_CAL2D_bank_merge_num)
    ...
    val u_bcontrol_begin = Module(new NV_soDLA_PDP_CORE_CAL2D_bubble_control_begin)
    ...
    val u_pnum_flush = Module(new NV_soDLA_PDP_CORE_CAL2D_pnum_flush)
    ...
    //-------------------------
    val u_pnum_updt = Module(new NV_soDLA_PDP_CORE_CAL2D_pnum_updt)
    ...
    val u_calculate_real_pooling_size = Module(new NV_soDLA_PDP_CORE_CAL2D_calculate_real_pooling_size)
    ...
    val u_mem_rd = Module(new NV_soDLA_PDP_CORE_CAL2D_mem_rd)
    ...
    //
    val u_rd_control_in_disable_time = Module(new NV_soDLA_PDP_CORE_CAL2D_rd_control_in_disable_time)
    ...
    //-----------------------------------------------------------
    val u_pad_v_and_div_kernel = Module(new NV_soDLA_PDP_CORE_CAL2D_pad_v_and_div_kernel)
...
object NV_soDLA_PDP_CORE_cal2dDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_PDP_CORE_cal2d())
}