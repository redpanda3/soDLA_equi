...
@chiselName
class NV_soDLA_PDP_WDMA_cmd(implicit val conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    val u_fifo = Module{new NV_soDLA_fifo(depth = 1, width = 80, ram_type = 1, distant_wr_req = true)}
...
object NV_soDLA_PDP_WDMA_cmdDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_PDP_WDMA_cmd())
}