...
@chiselName
class NV_soDLA_pdp(implicit val conf: nvdlaConfig) extends Module {
...
withReset(!io.nvdla_core_rstn){ 
    ...
    val u_reg = Module(new NV_soDLA_PDP_reg)
    val u_slcg_core = Array.fill(2){Module(new NV_soDLA_slcg(1, false))}
    val u_rdma = Module(new NV_soDLA_PDP_rdma)
    val u_nan = Module(new NV_soDLA_PDP_nan)
    val u_wdma = Module(new NV_soDLA_PDP_wdma)
    val u_core = Module(new NV_soDLA_PDP_core)
...
object NV_soDLA_pdpDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_pdp)
}
