...
@chiselName
class NV_soDLA_PDP_RDMA_ig(implicit conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    // DMA Interface
    //==============
    val u_NV_soDLA_PDP_RDMA_rdreq = Module(new NV_soDLA_DMAIF_rdreq(conf.NVDLA_DMA_RD_REQ))
    u_NV_soDLA_PDP_RDMA_rdreq.io.nvdla_core_clk := io.nvdla_core_clk
    if(conf.NVDLA_SECONDARY_MEMIF_ENABLE){
    io.pdp2cvif_rd_req_pd.get <> u_NV_soDLA_PDP_RDMA_rdreq.io.cvif_rd_req_pd.get
    }
    io.pdp2mcif_rd_req_pd <> u_NV_soDLA_PDP_RDMA_rdreq.io.mcif_rd_req_pd

    u_NV_soDLA_PDP_RDMA_rdreq.io.dmaif_rd_req_pd.valid := dma_rd_req_vld
    dma_rd_req_rdy := u_NV_soDLA_PDP_RDMA_rdreq.io.dmaif_rd_req_pd.ready
    u_NV_soDLA_PDP_RDMA_rdreq.io.dmaif_rd_req_pd.bits := dma_rd_req_pd

    u_NV_soDLA_PDP_RDMA_rdreq.io.reg2dp_src_ram_type := io.reg2dp_src_ram_type

}}


object NV_soDLA_PDP_RDMA_igDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_SDP_RDMA_ig)
}
