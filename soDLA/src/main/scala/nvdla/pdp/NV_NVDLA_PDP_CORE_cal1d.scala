...
@chiselName
class NV_soDLA_PDP_CORE_cal1d(implicit val conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    val pipe_0 = Module{new NV_soDLA_IS_pipe(conf.PDP_UNIT1D_BW + 15)}
    ...
    val u_NV_soDLA_PDP_cal1d_info_fifo = Module(new NV_soDLA_fifo(depth=8, width=12, ram_type=0))
    u_NV_soDLA_PDP_cal1d_info_fifo.io.clk := io.nvdla_core_clk
    u_NV_soDLA_PDP_cal1d_info_fifo.io.wr_pvld := pdp_info_in_pvld
    pdp_info_in_prdy := u_NV_soDLA_PDP_cal1d_info_fifo.io.wr_prdy
    u_NV_soDLA_PDP_cal1d_info_fifo.io.wr_pd := pdp_info_in_pd
    val pdp_info_out_pvld = u_NV_soDLA_PDP_cal1d_info_fifo.io.rd_pvld 
    u_NV_soDLA_PDP_cal1d_info_fifo.io.rd_prdy := pdp_info_out_prdy
    val pdp_info_out_pd = u_NV_soDLA_PDP_cal1d_info_fifo.io.rd_pd
    u_NV_soDLA_PDP_cal1d_info_fifo.io.pwrbus_ram_pd := io.pwrbus_ram_pd
    ...
    val unit1d = Array.fill(8){Module(new NV_soDLA_PDP_CORE_unit1d)}
    ...
    val u_pad = Module(new NV_soDLA_VEC_padder(vector_len = conf.NVDLA_PDP_THROUGHPUT, data_width = conf.NVDLA_BPE+3))
    for(i <- 0 to conf.NVDLA_PDP_THROUGHPUT-1){
...
object NV_soDLA_PDP_CORE_cal1dDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_PDP_CORE_cal1d())
}