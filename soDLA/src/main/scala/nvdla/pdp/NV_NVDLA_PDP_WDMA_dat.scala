...
@chiselName
class NV_soDLA_PDP_WDMA_dat(implicit val conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    //==============
    val u_dat_fifo = Array.fill(conf.ATMM_NUM){Array.fill(conf.BATCH_PDP_NUM){Module(new NV_soDLA_fifo(depth = 32, width = conf.PDPBW, ram_type = 0, distant_wr_req = false))}}
...
object NV_soDLA_PDP_WDMA_datDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_PDP_WDMA_dat())
}