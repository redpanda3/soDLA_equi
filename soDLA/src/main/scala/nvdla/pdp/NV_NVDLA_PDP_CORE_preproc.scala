...
@chiselName
class NV_soDLA_PDP_CORE_preproc(implicit val conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    val is_pipe0 = Module{new NV_soDLA_IS_pipe(conf.NVDLA_PDP_ONFLY_INPUT_BW + 1)}
    ...
    if(conf.SDP_THROUGHPUT > conf.NVDLA_PDP_THROUGHPUT){
        ...
        val u_ro_fifo = Array.fill(k){Module(new NV_soDLA_fifo(
                               depth = 4,
                               width = conf.PDPBW,
                               ram_type = 0,
...
object NV_soDLA_PDP_CORE_preprocDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_PDP_CORE_preproc())
}