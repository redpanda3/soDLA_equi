...
@chiselName
class NV_soDLA_PDP_RDMA_eg(implicit val conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    ///////////////////////////////////////////////////////////////////////////////////
    val NV_soDLA_PDP_RDMA_rdrsp = Module(new NV_soDLA_DMAIF_rdrsp(conf.NVDLA_PDP_MEM_RD_RSP))
    NV_soDLA_PDP_RDMA_rdrsp.io.nvdla_core_clk := io.nvdla_core_clk

    NV_soDLA_PDP_RDMA_rdrsp.io.mcif_rd_rsp_pd <> io.mcif2pdp_rd_rsp_pd

    if(conf.NVDLA_SECONDARY_MEMIF_ENABLE){
        NV_soDLA_PDP_RDMA_rdrsp.io.cvif_rd_rsp_pd.get <> io.cvif2pdp_rd_rsp_pd.get
    }
    ...
    val is_pipe_egg = Module(new NV_soDLA_IS_pipe(conf.NVDLA_PDP_MEM_RD_RSP))
    is_pipe_egg.io.clk := io.nvdla_core_clk

    is_pipe_egg.io.vi := NV_soDLA_PDP_RDMA_rdrsp.io.dmaif_rd_rsp_pd.valid
    NV_soDLA_PDP_RDMA_rdrsp.io.dmaif_rd_rsp_pd.ready := is_pipe_egg.io.ro
    is_pipe_egg.io.di := NV_soDLA_PDP_RDMA_rdrsp.io.dmaif_rd_rsp_pd.bits
    ...
    val u_lat_fifo = Module{new NV_soDLA_fifo(
    ...
    val is_pipe0 = Module(new NV_soDLA_IS_pipe(conf.NVDLA_PDP_BWPE*conf.NVDLA_PDP_THROUGHPUT+17))
...
object NV_soDLA_PDP_RDMA_egDriver extends App {
    implicit val conf: nvdlaConfig = new nvdlaConfig
    chisel3.Driver.execute(args, () => new NV_soDLA_PDP_RDMA_eg())
}
