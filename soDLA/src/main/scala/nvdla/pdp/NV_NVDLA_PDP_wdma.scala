...
@chiselName
class NV_soDLA_PDP_wdma(implicit val conf: nvdlaConfig) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    //==============
    val pipe_0 = Module(new NV_soDLA_IS_pipe(conf.PDPBW))
    ...
    // Instance CMD
    //==============
    val u_dat = Module(new NV_soDLA_PDP_WDMA_dat)
    ...
    // Instance CMD
    //==============
    val u_cmd = Module(new NV_soDLA_PDP_WDMA_cmd)
    ...
    val u_NV_soDLA_PDP_WDMA_wr = Module(new NV_soDLA_DMAIF_wr(conf.NVDLA_PDP_MEM_WR_REQ))
    u_NV_soDLA_PDP_WDMA_wr.io.nvdla_core_clk := io.nvdla_core_clk
    u_NV_soDLA_PDP_WDMA_wr.io.reg2dp_dst_ram_type := io.reg2dp_dst_ram_type

    u_NV_soDLA_PDP_WDMA_wr.io.dmaif_wr_req_pd.valid := dma_wr_req_vld
    dma_wr_req_rdy := u_NV_soDLA_PDP_WDMA_wr.io.dmaif_wr_req_pd.ready
    u_NV_soDLA_PDP_WDMA_wr.io.dmaif_wr_req_pd.bits := dma_wr_req_pd
    val dma_wr_rsp_complete = u_NV_soDLA_PDP_WDMA_wr.io.dmaif_wr_rsp_complete

    io.pdp2mcif_wr_req_pd <> u_NV_soDLA_PDP_WDMA_wr.io.mcif_wr_req_pd
    u_NV_soDLA_PDP_WDMA_wr.io.mcif_wr_rsp_complete := io.mcif2pdp_wr_rsp_complete


    if(conf.NVDLA_SECONDARY_MEMIF_ENABLE){
        io.pdp2cvif_wr_req_pd.get <> u_NV_soDLA_PDP_WDMA_wr.io.cvif_wr_req_pd.get
        u_NV_soDLA_PDP_WDMA_wr.io.cvif_wr_rsp_complete.get := io.cvif2pdp_wr_rsp_complete.get
    }
...
object NV_soDLA_PDP_wdmaDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_PDP_wdma())
}