...
@chiselName
class NV_soDLA_glb(implicit val conf: nvdlaConfig) extends Module {
...
//             └──┴──┘       └──┴──┘ 
    ...
    val u_csb = Module(new NV_soDLA_GLB_csb)
    val u_ic = Module(new NV_soDLA_GLB_ic)
...
object NV_soDLA_glbDriver extends App {
  implicit val conf: nvdlaConfig = new nvdlaConfig
  chisel3.Driver.execute(args, () => new NV_soDLA_glb())
}
